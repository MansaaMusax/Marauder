...
def build_faction():
    ...
    try:
        ...
        print("[Marauder Build] Could not load build config file. Error {}".format(
            e.Message))
        sys.exit(1)
    ...
    if restore_exit == 0:
        ...
        if build_config["Version"] == "NET35":
            ...
            print("[Marauder Build] Could not find a match for version: {}".format(
                build_config["Version"]))
            sys.exit(1)

        # Setup Debug vs Release and build
        if configuration == "Debug":
            build_cmd = "msbuild Marauder.csproj /t:Build /p:Configuration=Debug /p:OutputType=exe /p:TargetFrameworkVersion={}".format(
                version)
        else:
            build_cmd = "msbuild Marauder.csproj /t:Build /p:Configuration=Release /p:Optimize=true /p:OutputType=Winexe /p:TargetFrameworkVersion={}".format(
                version)
