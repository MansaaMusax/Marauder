...
def get_payload_name(payload_id: str):
    query = """
query Payload {
  payload_type(where: {id: {_eq: "PAYLOAD_ID"}}) {
    name
  }
}
""".replace("PAYLOAD_ID", payload_id)
    ...
    try:
        result = current_app.faction.client.graphql.execute(gql(query))
    ...
    if result:
        try:
            if len(result['payload_type']) > 0:
                return result['payload_type'][0]['name']
        except Exception as e:
